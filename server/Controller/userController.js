...
const registerUser = async (req, res) => {
    ...
    if (!validator.isStrongPassword(password)) return res.status(400).json("Password must be a stroing password")
...
const loginUser = async (req,res)=>{
    ...
    try{
        ...
        if(!isValid) return res.status(400).json("Incorrert Password")
        const token = createToken(user._id)
